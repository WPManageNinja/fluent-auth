    ...
    <div class="box dashboard_box box_narrow">
      ...
      >
        ...
        <div class="box_actions">
          <el-button @click="applyRecommended()" size="small">{{
            $t('apply_recommended')
          }}</el-button>
        </div>
      </div>
        ...
        <el-form :data="settings" label-position="top">
          <div class="fls_login_settings">
            ...
            <el-form-item>
              <el-checkbox
                true-label="yes"
                false-label="no"
                v-model="settings.disable_app_login"
              >
                {{ $t('disable_app_login') }}
              </el-checkbox>
            </el-form-item>
            <el-form-item>
              <el-checkbox
                true-label="yes"
                false-label="no"
                v-model="settings.disable_users_rest"
              >
                {{ $t('disable_rest_user') }}
              </el-checkbox>
            </el-form-item>
          ...
          <div class="fls_login_settings">
            ...
            <el-form-item class="fls_switch">
              <el-switch
                v-model="settings.enable_auth_logs"
                active-value="yes"
                inactive-value="no"
              />
            ...
            <template v-else>
              <el-row :gutter="30">
                <el-col :md="12" :sm="24">
                  <el-form-item
                    :label="$t('login_try_limit') + ' ' + $t('minutes')"
                  >
                    <el-input
                      type="number"
                      v-model="settings.login_try_limit"
                    />
                    <p>
                      {{ $t('login_how_many') }}
                      {{ settings.login_try_timing }} {{ $t('minutes') }}
                    </p>
                  </el-form-item>
                </el-col>
                <el-col :md="12" :sm="24">
                  <el-form-item
                    :label="$t('login_try_time_limit') + ' ' + $t('minutes')"
                  >
                    <el-input
                      type="number"
                      v-model="settings.login_try_timing"
                    />
                    <p>
                      {{ $t('user_fails_to_login') }}
                      {{ settings.login_try_limit }} {{ $t('times_within') }}
                      {{ settings.login_try_timing }} {{ $t('minutes') }}
                      {{ $t('system_will_block') }}
                      {{ settings.login_try_timing }} {{ $t('minutes') }}.
                    </p>
          ...
          <div
            class="fls_login_settings"
            v-if="settings.enable_auth_logs == 'yes'"
          >
            <h3>{{ $t('extended_login_options') }}</h3>

            <div
              class="fls_inner_group"
              :class="'fls_inner_group_' + settings.magic_login"
            >
              <el-form-item>
                <el-checkbox
                  v-model="settings.magic_login"
                  true-label="yes"
                  false-label="no"
                >
              ...
              >
                ...
                <el-select
                  placeholder="Select disable roles"
                  clearable
            ...
            <div
              class="fls_inner_group"
              :class="'fls_inner_group_' + settings.email2fa"
            >
              <el-form-item>
                <el-checkbox
                  v-model="settings.email2fa"
                  true-label="yes"
                  false-label="no"
                >
              ...
              >
                ...
                <el-select
                  :placeholder="$t('enabled_for_all_user_roles')"
                  clearable
                  v-model="settings.email2fa_roles"
                  :multiple="true"
                >
                  <el-option
                    v-for="role in user_roles"
          ...
          <div class="fls_login_settings">
            ...
            <el-row :gutter="30">
              <el-col :sm="24" :md="12">
                <el-form-item>
                  ...
                  <el-select
                    clearable
                    v-model="settings.notification_user_roles"
                    :multiple="true"
                  >
                    <el-option
                      v-for="role in user_roles"
              ...
              <el-col :sm="24" :md="12">
                <el-form-item
                  v-if="
                    settings.notification_user_roles.length ||
                    settings.notify_on_blocked == 'yes' ||
                    settings.digest_summary
                  "
                >
                  <template #label>
            ...
            <el-row :gutter="30">
              <el-col :sm="24" :md="12">
                <el-form-item class="fls_switch">
                  <el-switch
                    v-model="settings.notify_on_blocked"
                    active-value="yes"
                    inactive-value="no"
                  />
                ...
                <el-form-item label="Send Digest Email Report Summary">
                  <el-select v-model="settings.digest_summary">
                    <el-option
                      value=""
                      label="Do not send digest email summary"
                    ></el-option>
                    <el-option
                      v-for="(day, dayName) in digest_items"
                      :value="dayName"
                      :label="day"
                    ></el-option>
                  </el-select>
              ...
              <el-col :sm="24" :md="12">
                <el-form-item>
                  ...
                  <el-input
                    v-model="settings.auto_delete_logs_day"
                    type="number"
                    :min="0"
                  />
                </el-form-item>
            ...
            <el-row :gutter="30">
              ...
              <el-col :sm="24" :md="12">
                <el-form-item v-if="settings.disable_admin_bar == 'yes'">
                  ...
                  <el-select
                    clearable
                    v-model="settings.disable_bar_roles"
                    :multiple="true"
                  >
                    <el-option
                      v-for="(role, roleId) in low_level_roles"
                      :value="roleId"
                      :label="role"
                      :key="roleId"
                    ></el-option>
          ...
          <div class="fls_errors" v-if="errors">
            <ul>
              <li v-for="(error, errorKey) in errors" :key="errorKey">
                {{ convertToText(error) }}
              </li>
            </ul>
          </div>
...
export default {
  ...
  methods: {
    ...
    applyRecommended() {
      ...
      this.$notify.success(
        'Recommended settings has been applied. Please review and save the settings',
      );
    },
  },
  mounted() {
